...
```swift
  ...
  guard let databaseTmp = database
   else { return }

  document = databaseTmp.createDocument()
  ...
  guard let documentTmp = document
   else { return }

  print("Document ID :: \(documentTmp.documentID)")
  print("Learning \(documentTmp.property(forKey: "sdk")!)")

  // Create replicators to push & pull changes to & from Sync Gateway
  let url = URL(string: "http://localhost:4984/hello")!
  let push = databaseTmp.createPushReplication(url)
  let pull = databaseTmp.createPullReplication(url)

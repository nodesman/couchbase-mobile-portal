...
Documents have changed a lot. The data model is still the same — a JSON object with a fixed ID string — but the API has absorbed ideas from iOS' `CBLModel` to make it easier to work with.

### Mutability

The biggest change is that {% st CBLDocument, CBLDocument, IDocument, IDocument %} properties are now mutable. Instead of having to make a mutable copy of the properties dictionary, update it, and then save it back to the document, you can now modify individual properties in place and then save.

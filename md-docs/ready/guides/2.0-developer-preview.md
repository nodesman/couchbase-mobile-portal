...
TBD: Are we going to have subdocuments in DP1?
...
CBLSubDocument, like CBLDocument, inherits from CBLProperties. That means it has the same set of type-specific accessors discussed in the previous section. Like CBLDocument, it's mutable, so you can make changes in-place.
The difference is that a subdocument doesn't have its own ID. It's not a first-class entity in the database, it's just a nested object within the document's JSON. It can't be saved individually; changes are persisted when you save its document.
...
TBD: Add docs on blobs when the branch is merged

Attachments are not (re-)implemented yet. The new attachment API is still under design, but will be a lot more flexible and easier to use, letting you store “blobs” anywhere in a document's structure, even inside nested properties.
...
> We're still evaluating whether to support map/reduce in 2.0. We recognize that, although it has a learning curve, it can be very powerful. We appreciate your feedback on this.
...
> The query API does not yet support aggregation, i.e. functions like `AVG()` or operators like `GROUP BY`. This will be added in a future preview release.

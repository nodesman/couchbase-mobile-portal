...
|`revs_limit`|`integer`|Maximum depth to which a document's revision tree can grow. This setting can be used to reduce the maximum size of Sync Gateway's metadata. Be aware that if the value is too low it may result in unexpected behaviour when conflicts occur. The default is `1000`. To purge individual revisions it is recommended to use the `/{db}/_purge` endpoint instead.|

...
<xsl:stylesheet version="2.0"
        ...
        <xsl:variable name="groups">
            <xsl:for-each select="$docs">
                <group>
                    ...
                    <xsl:variable name="sets" select="ancestor::*[self::set or self::guide or self::class]"/>
                    <xsl:choose>
                        <xsl:when test="$sets">
    ...
    <xsl:template match="*" mode="__search-index">
        <xsl:variable name="docs" select="descendant-or-self::*[self::article or self::lesson or self::page or self::xhtml-page or self::class[../parent::package]]"/>
        ...
        <xsl:variable name="groups">
            <xsl:for-each select="$docs">
                <group>
                    ...
                    <xsl:for-each select="ancestor-or-self::*[self::set][last()]">
                        <xsl:if test="position() > 1">
                            <xsl:text>,</xsl:text>
                        </xsl:if>
